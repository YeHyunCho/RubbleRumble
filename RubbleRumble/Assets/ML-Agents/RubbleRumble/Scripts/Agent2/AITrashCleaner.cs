//본 코드
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using Unity.MLAgents;
using Unity.MLAgents.Actuators;
using Unity.MLAgents.Sensors;

// 필요한 컴포넌트 선언
[RequireComponent(typeof(AgentMotorK))]
[RequireComponent(typeof(TrashInteractionManager))]
[RequireComponent(typeof(AICleanerBase))]
public class AITrashCleaner : Agent
{
    // ─────────────── 참조 변수 ────────────────
    [Header("References")]
    public AgentMotorK motor;                         // 이동 처리용 모터
    public TrashInteractionManager interaction;       // 쓰레기와의 상호작용 매니저
    public AICleanerBase cleaner;                     // 에이전트의 도구 및 쓰레기 상태 관리

    // ─────────────── 설정값 ────────────────
    [Header("Decision Parameters")]
    public int decisionInterval = 1;                  // 행동 결정을 내리는 간격
    public float stepPenalty = -0.01f;                // 매 스텝마다 주는 시간 지연 벌점

    // ─────────────── 보상 값 ────────────────
    [Header("Reward Values")]
    public float rPickUpCan = 0.5f;                   // 캔을 주웠을 때 보상
    public float rReachBin = 0.3f;                    // 분리수거장에 도달했을 때 보상
    public float rThrowAway = 1.0f;                   // 쓰레기를 버렸을 때 보상
    public float rApproachTarget = 0.001f;             // 목표물에 접근했을 때 보상
    public float rRetreatPenalty = -0.0011f;            // 목표물에서 멀어졌을 때 벌점
    public float rWashMop = 0.7f;                        // ★ 대걸레 세척 보상 추가
    public float rPlaceBox = 0.6f;                         // ★ Box를 책상에 내려놓으면 보상


    // 내부 상태 추적용 변수들
    private bool _pickedUp = false;
    private bool _reachedBin = false;
    private bool _threwAway = false;
    private int _prevScore = 0;
    private float _prevBinDist = 0f;
    private float _prevPaperBinDist = 0f;
    private float _prevSinkDist = 0f;
    private float _prevDeskDist = 0f;
    private DecisionRequester _requester;
    private bool _mopWasDirty = false;      // ★ 새 변수
    private int _prevMopUse = 0;

    private bool _placedBox = false; // 같은 상자에 중복 보상 방지
    private float _deskHoldTime = 0f;    // 책상 근처 머문 시간

    //쿨타임
    private float _distRewardCooldown = 0.5f;  // 쿨타임 간격 (초)
    private float _lastDistRewardTime = -999f; // 마지막 보상 시간
    // 에이전트 초기화 시 호출
    public override void Initialize()
    {
        base.Initialize();
        if (cleaner == null) cleaner = GetComponent<AICleanerBase>();
        if (motor == null) motor = GetComponent<AgentMotorK>();
        if (interaction == null) interaction = FindObjectOfType<TrashInteractionManager>();

        _requester = GetComponent<DecisionRequester>();
        _requester.DecisionPeriod = Mathf.Max(1, decisionInterval);
        _requester.TakeActionsBetweenDecisions = false;
    }

    // 에피소드가 시작될 때 호출
    public override void OnEpisodeBegin()
    {
        var rb = GetComponent<Rigidbody>();
        if (rb != null)
        {
            rb.velocity = Vector3.zero;
            rb.angularVelocity = Vector3.zero;
        }

        // 시작 위치 및 회전 초기화

        transform.rotation = Quaternion.Euler(0f, 180f, 0f);

        // 환경 및 내부 상태 초기화
        StageManager.Instance?.TimeReset();
        MapManager.Instance?.ResetEnvironment();
        RetryTestBtn.OnRetryTestButtonCliked();

        _pickedUp = false;
        _reachedBin = false;
        _threwAway = false;
        _prevScore = 0;
        cleaner.ResetHeldTrash();

        _prevPaperBinDist = GetClosestTargetDistance("TBpaper");
        _prevBinDist = GetClosestTargetDistance("TBcan");
        _prevSinkDist = GetClosestTargetDistance("Sink");
        _prevDeskDist = GetClosestTargetDistance("Desk");
        _mopWasDirty = false;
        _prevMopUse = 0;

        _placedBox = false;
        _deskHoldTime = 0f;
    }

    // 매 프레임마다 실행되는 행동 요청
    private void FixedUpdate()
    {
        if (_requester == null)
            RequestDecision();
    }

    // 행동 수행 및 보상 처리
    public override void OnActionReceived(ActionBuffers actions)
    {
        AddReward(stepPenalty); // 시간 지연 페널티

        // ─ 이동 처리 ─
        int move = actions.DiscreteActions[0];
        Vector3 dir = move switch
        {
            1 => Vector3.forward,
            2 => Vector3.back,
            3 => Vector3.left,
            4 => Vector3.right,
            5 => (Vector3.forward + Vector3.left),
            6 => (Vector3.forward + Vector3.right),
            7 => (Vector3.back + Vector3.left),
            8 => (Vector3.back + Vector3.right),
            _ => Vector3.zero
        };
        if (dir != Vector3.zero) dir.Normalize();
        motor.Move(dir); // 모터를 통해 이동

        // ─ 상호작용 처리 ─
        int act = actions.DiscreteActions[1];
        switch (act)
        {
            case 1: cleaner.UseToolPublic(); break;         // 도구 사용
            case 2: cleaner.TryThrowAwayPublic(); break;    // 쓰레기 버리기

            case 3: cleaner.EquipToolPublic(0); break;// 맨손 장착
            case 4: cleaner.EquipToolPublic(1); break;// 빗자루 장착
            case 5: cleaner.EquipToolPublic(2); break;// 대걸레(Mop) 장착
            case 6: cleaner.TryPlaceTrashOnWorkbenchPublic(); break; //박스 올리기
        }

        // ─ 보상 처리 ─
        if (!_pickedUp && cleaner.IsHoldingTrash)
        {
            _pickedUp = true;
            AddReward(rPickUpCan);
        }

        float binDist = GetClosestTargetDistance("TBcan");
        if (!_reachedBin && binDist < 1f)
        {
            _reachedBin = true;
            AddReward(rReachBin);
        }

        if (_pickedUp && !_threwAway && !cleaner.IsHoldingTrash)
        {
            _threwAway = true;
            AddReward(rThrowAway);
        }

        // 점수 변화 기반 보상 (1점 이상은 무시)
        int cur = StageManager.Instance != null ? StageManager.Instance.AIScore : _prevScore;
        if (cur > _prevScore)
        {
            int delta = cur - _prevScore;
            AddReward(Mathf.Clamp(delta, 0, 1)); // 보상 최대 1점만
        }
        _prevScore = cur;


        // 목표물 접근/후퇴 감지 보상

        // ─ 목표물 접근/후퇴 감지 보상 ─
        GameObject held = cleaner.HeldTrash;  //손에 들고있는 것 체크

        if (held != null)
        {
            string tag = held.tag;
            float curDist = 0f;
            float prevDist = 0f;
            string targetTag = null;

            // 어떤 대상에 대해 거리 보상을 줄 것인지 결정
            if (tag == "Can") //캔을 들고있을 경우
            {
                targetTag = "TBcan";
                curDist = GetClosestTargetDistance(targetTag);
                prevDist = _prevBinDist;
                _prevBinDist = curDist;
            }
            else if (tag == "Box")
            {
                targetTag = "Desk";
                curDist = GetClosestTargetDistance(targetTag);
                prevDist = _prevDeskDist;
                _prevDeskDist = curDist;
            }
            else if (tag == "UnfoldedBox")
            {
                targetTag = "TBpaper";
                curDist = GetClosestTargetDistance(targetTag);
                prevDist = _prevPaperBinDist;
                _prevPaperBinDist = curDist;
            }

            // HeldTrash의 UseCount 확인
            var usage = held.GetComponent<Mop>();
            if (usage != null && usage.GetUseCount() >= 2)
            {
                targetTag = "Sink";
                curDist = GetClosestTargetDistance(targetTag);
                prevDist = _prevSinkDist;
                _prevSinkDist = curDist;
            }

            float now = Time.time; //0.5초에 한번씩 체크하며 보상
            if (now - _lastDistRewardTime >= _distRewardCooldown && targetTag != null)
            {
                if (curDist < prevDist)
                {
                    AddReward(rApproachTarget);
                    _lastDistRewardTime = now;
                }
                else if (curDist > prevDist)
                {
                    AddReward(rRetreatPenalty);
                    _lastDistRewardTime = now;
                }
            }
            // ─────────────────────────────────────────────────────
            // 3. Mop 세척 보상
            // ─────────────────────────────────────────────────────
            var mop = cleaner.HeldTrash?.GetComponent<Mop>();
            if (mop != null)
            {
                int curUse = mop.GetUseCount();                    // 현재 오염 단계(0~2)  :contentReference[oaicite:1]{index=1}

                // 더러웠다가(useCount>=2) 0으로 리셋된 프레임 = 세척 성공
                if (_mopWasDirty && curUse == 0 && _prevMopUse >= 2)
                {
                    AddReward(rWashMop);                           // ★ 보상 지급
                    _mopWasDirty = false;                          // 상태 리셋
                }

                // useCount가 2 이상이면 ‘더러운 상태’ 플래그 켜 두기
                if (curUse >= 2) _mopWasDirty = true;

                _prevMopUse = curUse;                              // 다음 프레임 대비
            }
            else
            {
                // 손에서 내려놓은 순간 상태 초기화 (옵션)
                _mopWasDirty = false;
                _prevMopUse = 0;
            }



        }

        if (cleaner.IsHoldingTrash)                // 손에 뭔가 들고 있을 때만
        {
            if (held != null && held.CompareTag("Box"))
            {
                float deskDist = GetClosestTargetDistance("Desk");

                if (deskDist < 1.0f)               // 책상 반경(1m) 안
                {
                    _deskHoldTime += Time.deltaTime;

                    if (_deskHoldTime >= 2f && !_placedBox)
                    {
                        // 책상(WorkbenchArea) 트리거 안이므로 바로 내려놓기
                        cleaner.TryPlaceTrashOnWorkbenchPublic();   // 자동 배치
                        AddReward(rPlaceBox);                       // 보상 지급
                        _placedBox = true;                         // 중복 방지
                        _deskHoldTime = 0f;                         // 타이머 리셋
                    }
                }
                else
                {
                    _deskHoldTime = 0f;      // 책상에서 벗어나면 타이머 초기화
                }
            }
        }



    }

    // 휴리스틱(키보드 테스트용)
    public override void Heuristic(in ActionBuffers actionsOut)
    {
        var d = actionsOut.DiscreteActions;

        // 이동 키 입력
        if (Input.GetKey(KeyCode.W) && Input.GetKey(KeyCode.A)) d[0] = 5;
        else if (Input.GetKey(KeyCode.W) && Input.GetKey(KeyCode.D)) d[0] = 6;
        else if (Input.GetKey(KeyCode.S) && Input.GetKey(KeyCode.A)) d[0] = 7;
        else if (Input.GetKey(KeyCode.S) && Input.GetKey(KeyCode.D)) d[0] = 8;
        else if (Input.GetKey(KeyCode.W)) d[0] = 1;
        else if (Input.GetKey(KeyCode.S)) d[0] = 2;
        else if (Input.GetKey(KeyCode.A)) d[0] = 3;
        else if (Input.GetKey(KeyCode.D)) d[0] = 4;
        else d[0] = 0;

        // 상호작용 키 입력
        if (Input.GetKey(KeyCode.E)) d[1] = 1;
        else if (Input.GetKey(KeyCode.R)) d[1] = 2;
        else if (Input.GetKey(KeyCode.Alpha1)) d[1] = 5;
        else d[1] = 0;
    }

    // 센서 관측 추가
    public override void CollectObservations(VectorSensor sensor)
    {
        // 쓰레기를 들고 있는지 여부
        sensor.AddObservation(cleaner.IsHoldingTrash ? 1f : 0f);

        // 캔 방향 및 거리
        Vector3 dirCan;
        float distCan = GetClosestTargetDistance("Can", out dirCan);
        sensor.AddObservation(dirCan);
        sensor.AddObservation(distCan);

        // Dust (일반 쓰레기)
        Vector3 dirDustTrash;
        float distDustTrash = GetClosestTargetDistance("Dust", out dirDustTrash);
        sensor.AddObservation(dirDustTrash);
        sensor.AddObservation(distDustTrash);

        // Box (종이 박스)
        Vector3 dirBox;
        float distBox = GetClosestTargetDistance("Box", out dirBox);
        sensor.AddObservation(dirBox);
        sensor.AddObservation(distBox);

        // Water (물 오브젝트)
        Vector3 dirWater;
        float distWater = GetClosestTargetDistance("Water", out dirWater);
        sensor.AddObservation(dirWater);
        sensor.AddObservation(distWater);


        // 캔용 분리수거장 방향 및 거리
        Vector3 dirBin;
        float distBin = GetClosestTargetDistance("TBcan", out dirBin);
        sensor.AddObservation(dirBin);
        sensor.AddObservation(distBin);

        // 벽까지 가장 가까운 거리
        sensor.AddObservation(GetClosestTargetDistance("Wall"));

        //대걸레가 더러운가 아닌가 체크하여 센서 추가
        int mopUse = 0;
        var mop = cleaner.HeldTrash?.GetComponent<Mop>();
        if (mop != null) mopUse = Mathf.Clamp(mop.GetUseCount(), 0, 2);
        sensor.AddObservation(mopUse / 2f);   // 0.0, 0.5, 1.0

        // ─ 추가 오브젝트 감지 ─
        // 일반 쓰레기통 (TBdust)
        Vector3 dirDust;
        float distDust = GetClosestTargetDistance("TBdust", out dirDust);
        sensor.AddObservation(dirDust);
        sensor.AddObservation(distDust);

        // 종이용 쓰레기통 (TBpaper)
        Vector3 dirpaper;
        float distpaper = GetClosestTargetDistance("TBpaper", out dirpaper);
        sensor.AddObservation(dirpaper);
        sensor.AddObservation(distpaper);

        // 책상 (Desk)
        Vector3 dirDesk;
        float distDesk = GetClosestTargetDistance("Desk", out dirDesk);
        sensor.AddObservation(dirDesk);
        sensor.AddObservation(distDesk);

        // 싱크대 (Sink)
        Vector3 dirSink;
        float distSink = GetClosestTargetDistance("Sink", out dirSink);
        sensor.AddObservation(dirSink);
        sensor.AddObservation(distSink);


        //들고있는 쓰레기 체크
        float[] heldType = new float[4]; // 0: 없음, 1: Can, 2: Box, 3: UnfoldedBox

        if (cleaner.HeldTrash != null)
        {
            string tag = cleaner.HeldTrash.tag;
            if (tag == "Can") heldType[1] = 1f;
            else if (tag == "Box") heldType[2] = 1f;
            else if (tag == "UnfoldedBox") heldType[3] = 1f;
        }
        else
        {
            heldType[0] = 1f; // 아무것도 안 들었을 때
        }

        foreach (var v in heldType)
            sensor.AddObservation(v);


    }

    // 가장 가까운 대상과의 거리 계산 (방향 무시)
    private float GetClosestTargetDistance(string tag)
        => GetClosestTargetDistance(tag, out _);

    // 가장 가까운 대상과의 거리 및 방향 계산
    private float GetClosestTargetDistance(string tag, out Vector3 direction)
    {
        direction = Vector3.zero;
        GameObject[] objs = GameObject.FindGameObjectsWithTag(tag);
        float min = float.MaxValue;
        Vector3 agentPos = transform.position;

        foreach (var o in objs)
        {
            float d = Vector3.Distance(agentPos, o.transform.position);
            if (d < min)
            {
                min = d;
                direction = (o.transform.position - agentPos).normalized;
            }
        }

        // 아무것도 없으면 거리 0 처리
        if (min == float.MaxValue) min = 0f;
        if (min == 0f) direction = Vector3.zero;
        return min;
    }
}
